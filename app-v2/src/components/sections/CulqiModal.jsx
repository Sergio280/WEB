...
import { CULQI_CONFIG, USD_PRICES } from '../../data/culqi.js';
...
export default function CulqiModal({ planKey, onClose }) {
  ...
  // Precios en USD: fuente única en data/culqi.js, junto a los slugs de checkout
  // de Lemon Squeezy. Antes estaban duplicados aquí y en Pricing.jsx.
  const [intlDuration, setIntlDuration] = useState('monthly'); // 'monthly' | 'yearly'
  const usdPrice = (USD_PRICES[planKey] || USD_PRICES.individual)[intlDuration];
  ...
  function handlePay() {
    ...
    setError('');
    // No se marca `processing` aquí: el checkout de Culqi se abre en su propia
    // ventana y el usuario puede cerrarla. El estado lo activa `onProcessing`,
    // que sólo se dispara cuando hay token y empieza el cobro de verdad.
    ...
    openCulqiCheckout({
      ...
      // El usuario cerró el checkout sin pagar: liberar el botón sin marcar error.
  ...
  // El AnimatePresence vive en el PADRE (Pricing.jsx): aquí dentro se desmontaba
  // junto con el modal y las animaciones `exit` nunca llegaban a reproducirse.
  return (
    <motion.div
    ...
    >
      <motion.div
        ...
        aria-modal="true"
        aria-label={badge}
        className="max-h-full w-full max-w-md overflow-y-auto rounded-2xl border border-white/10 bg-ink-800 shadow-glow-lg"
